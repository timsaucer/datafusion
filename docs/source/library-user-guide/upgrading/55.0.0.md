...
### `PhysicalOptimizerRule` and `PhysicalOptimizerContext` moved to `datafusion-session`

`PhysicalOptimizerRule` and `PhysicalOptimizerContext` now live in the `datafusion-session` crate. They are still re-exported from `datafusion-physical-optimizer` to reduce downstream churn, so existing imports such as `datafusion_physical_optimizer::PhysicalOptimizerRule` continue to work.
