...
### `PhysicalOptimizerRule`, `PhysicalOptimizerContext`, and `QueryPlanner` moved to `datafusion-session`

`PhysicalOptimizerRule`, `PhysicalOptimizerContext`, and `QueryPlanner` now live in the `datafusion-session` crate. They are still re-exported from their previous public locations to reduce downstream churn, so existing imports such as `datafusion_physical_optimizer::PhysicalOptimizerRule` and `datafusion::execution::context::QueryPlanner` continue to work.

New code can import them from `datafusion_session`:

```rust
use datafusion_session::{
    PhysicalOptimizerContext, PhysicalOptimizerRule, QueryPlanner,
};
```
...
### `ExecutionProps::subquery_indexes` and `subquery_results` removed
...
uncorrelated scalar-subquery state into expression lowering (turning a logical
`Expr` into a physical `Arc<dyn PhysicalExpr>`).

That state is now carried by a dedicated `SubqueryContext` threaded explicitly
through the lowering functions, so it no longer needs to live on
`ExecutionProps`. This removes the need for the physical planner to clone and
mutate a `SessionState` and is a prerequisite for letting the planner take
`&dyn Session`.

**Who is affected:**

- Code that read or wrote `execution_props.subquery_indexes` /
  `execution_props.subquery_results`. In practice only the physical planner ever
  populated these, so most callers are unaffected.

**Migration guide:**

If you populated these fields to lower `Expr::ScalarSubquery` yourself, build a
`SubqueryContext` and pass it to the new `_with_subquery_context` lowering
entry points instead:

```rust,ignore
use datafusion_expr::execution_props::SubqueryContext;
use datafusion_physical_expr::create_physical_expr_with_subquery_context;

// Before: mutate ExecutionProps, then call create_physical_expr
...
// After: build a SubqueryContext and thread it explicitly
let subquery_ctx = SubqueryContext::new(index_map, results.clone());
let phys = create_physical_expr_with_subquery_context(
    &expr,
    &schema,
    execution_props,
    &subquery_ctx,
...
Callers that never touched these fields need no change: the original
`create_physical_expr` (and the other lowering functions) keep their signatures
and behave exactly as before.

See [PR #22340](https://github.com/apache/datafusion/pull/22340) for details.
